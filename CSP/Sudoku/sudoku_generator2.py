...
class cell():
    """ Initilalizes cell object. A cell is a single box of a sudoku puzzle. 81 cells make up the body of a
        sudoku puzzle. Initializes puzzle with all possible answers available, solved to false, and position of cell
        within the
        sudoku puzzle"""
...
def sudokuGen():
    ...
    while len(cells) != 0:
        ...
        if choiceElement.solvedMethod() == False:  ##the actual setting of the cell
            ...
            for i in cells:  ##now we iterate through the remaining unset cells and remove the input if it's in the
                # same row, col, or box
...
def solver(sudoku, f=0):
    """ Input an incomplete Sudoku puzzle and solver method will return the solution to the puzzle. First checks to
    see if any obvious answers can be set
        then checks the rows columns and boxes for obvious solutions. Lastly the solver 'guesses' a random possible
        answer from a random cell and checks to see if that is a
        possible answer. If the 'guessed' answer is incorrect, then it removes the guess and tries a different answer
        in a different cell and checks for a solution. It does this until
        all of the cells have been solved. Returns a printed solution to the puzzle and the number of guesses that it
        took to complete the puzzle. The number of guesses is
        a measure of the difficulty of the puzzle. The more guesses that it takes to solve a given puzzle the more
        challenging it is to solve the puzzle"""
    if f > 900:
        return False
    guesses = 0
    ...
    while solvedCells != []:
        for n in solvedCells:
            ...
            for i in cells:  ##now we itterate through the remaing unset cells and remove the input if it's in the
                # same row, col, or box
...
def solve(sudoku, n=0):
    ...
    Returns True if the puzzle is solvable and
        false if otherwise"""
    if n < 30:
        s = solver(sudoku)
        if s != False:
